 ...
 *      境界        :TRUE
 *      境界でない  :FALSE
 * arg      enum ChType pre, now
 */
enum bool isBorder(enum ChType pre, enum ChType now);

int main(void){
    int c1;
    int c2;
    char euc_ch[3];
    enum ChType pre_type = S;
    enum ChType now_type;
    ...
    while((c1 = getchar()) != EOF){
        ...
        now_type = checkChType(c1, c2);
        if(isBorder(pre_type, now_type) == TRUE)
            printf("\n");
        printf("%s", euc_ch);
        pre_type = now_type;
    }
